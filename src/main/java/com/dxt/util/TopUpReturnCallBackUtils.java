...
public class TopUpReturnCallBackUtils {
        String url = "http://124.202.131.74:11111/svc/asyn/recharge/Callback";
        public String getHttpsResponse(TopUpCallEntity topUpCallEntity) throws ParseException, IOException{
            String appKey = "DXT";
            ...
            try {
                ...
                //请求头头部设置
                TopUpCallBackRequest request = new TopUpCallBackRequest();
                request.getHead().setAppKey(appKey);
                request.getHead().setMethod("recharge.callback");
                request.getHead().setReqTime(new Date());
                request.getHead().setVersion("V1.0");
                request.getHead().setTransactionId(transactionId);

                //请求体内容设置
                TopUpCallBackRequestBody body = new TopUpCallBackRequestBody();
                TopUpCallReq topUpCallReq = new TopUpCallReq();
                topUpCallReq.setTransactionId(topUpCallEntity.getTransactionId());
                topUpCallReq.setPlatTransactionId(topUpCallEntity.getPlatTransactionId());
                topUpCallReq.setDestCode(topUpCallEntity.getDestCode());
                topUpCallReq.setFeeAmount(topUpCallEntity.getFeeAmount());
                topUpCallReq.setStatus(topUpCallEntity.getStatus());
                topUpCallReq.setReturnCode(topUpCallEntity.getReturnCode());
                topUpCallReq.setErrorMsg(topUpCallEntity.getErrorMsg());
                ...
                body.setTopUpCallReq(topUpCallReq);
