...
public class PhotoSynchronizationService extends IBaseBusiService {
    ...
    public ReponseMessage execute(String reqInfo, String source) {
        ...
        try {
            ...
            map.put("in_openid",jsonObject.getString(PARAM_CARDNUMBER));
    ...
    private void checkPArms(JSONObject jsonObject) throws MyBusiException {
        if (!jsonObject.getString(PARAM_CARDTYPE).equals("1")){
            throw  new  MyBusiException("证件类型不符合要求");
        }
        if ("".equals(jsonObject.getString(PARAM_CARDNUMBER)) || jsonObject.getString(PARAM_CARDNUMBER).length()!=18) {
        ...
        if (Integer.valueOf(format1)>Integer.valueOf(jsonObject.getString(PARAM_FAILUREOFCARDTIME)))
        {
            throw  new MyBusiException("证件已过期");
        }
        if (IdCardVerification.IDCardValidate(jsonObject.getString(PARAM_CARDNUMBER))==false){
            throw  new  MyBusiException("证件号码有误！");
        }

        if (!CommonUtils.inValid(jsonObject.getString(PARAM_PHONE))){
            throw new MyBusiException("号码格式不正确");
        }else if ("".equals(jsonObject.getString(PARAM_PHONE)) || "".equals(jsonObject.getString(PARAM_IDCARDFRONT))  ||"".equals(jsonObject.getString(PARAM_REVERSEIDCARD_ICCID)) ||"".equals(jsonObject.getString(PARAM_BAREHEADPHOTO))  || "".equals(jsonObject.getString(PARAM_PLATFORM))) {
        ...
        if (jsonObject.getInteger(PARAM_ISPAS)==null ){
            throw  new  MyBusiException("参数不能为空");
        }

        if (jsonObject.getInteger(PARAM_ISPAS)!=1 && jsonObject.getInteger(PARAM_ISPAS)!=0){
            throw  new  MyBusiException("参数非法");
        }

        if (!jsonObject.getString(PARAM_PLATFORM).equals("9") && !jsonObject.getString(PARAM_PLATFORM).equals("10")){
